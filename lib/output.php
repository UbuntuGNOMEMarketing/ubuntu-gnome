...
add_action( 'wp_enqueue_scripts', 'ubuntu_gnome_css' );
/**
...
* @since 1.0
*/
function ubuntu_gnome_css() {

	$handle  = defined( 'CHILD_THEME_NAME' ) && CHILD_THEME_NAME ? sanitize_title_with_dashes( CHILD_THEME_NAME ) : 'child-theme';

	$opts = apply_filters( 'ubuntu-gnome_images', array( '1' ) );
...
foreach ( $settings as $section => $value ) { 
	...
	$css .= ( ! empty( $section ) && ! empty( $background ) ) ? sprintf( '.banner-%s { %s }', $section, $background ) : '';
